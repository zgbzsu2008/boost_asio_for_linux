...
class waitable_timer_service
 ...
 private:
  using service_impl_type =
      deadline_timer_service<detail::chrono_time_traits<Clock, WaitTraits>>;

 private:
  using service_impl_type = deadline_timer_service<traints_type>;

 public:
  using implementation_type = typename service_impl_type::implementation_type;

  explicit waitable_timer_service(io_context& ioc)
      : detail::service_base<deadline_timer_service<TimeType, TimeTraits>>(ioc),
  ...
  std::size_t cancle(service_impl_type& impl, std::error_code& ec) {
    return service_impl_.cancle(impl, ec);
  }

  std::size_t cancle_one(service_impl_type& impl, std::error_code& ec) {
    return service_impl_.cancle_one(impl, ec);
  }
  ...
  typename detail::async_result_helper<WaitHandler,
                                       void(std::error_code ec)>::result_type
  async_wait(implementation_type& impl, WaitHandler&& handler) {
