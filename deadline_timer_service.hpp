    ...
    : public boost::asio::detail::service_base<deadline_timer_service<TimeType, TimeTraits> >
{
 ...
 public:
  ...
  std::size_t cancel_one(implementation_type& impl, std::error_code& ec)
  {
    return service_impl_.cancel_one(impl, ec);
  }

  time_point expires_at(const implementation_type& impl) const { return service_impl_.expiry(impl); }

  std::size_t expires_at(implementation_type& impl, const time_point& expiry_time, std::error_code& ec)
  {
  ...
  std::size_t expires_from_now(implementation_type& impl, const duration& expiry_time,
                               std::error_code& ec)
  {
    return service_impl_.expires_after(impl, expiry_time, ec);
  }

  void wait(implementation_type& impl, std::error_code& ec) { service_impl_.wait(impl, ec); }

  template <typename WaitHandler>
  typename detail::async_result_helper<T, void(std::error_code)>::result_type async_wait(
      implementation_type& impl, BOOST_ASIO_MOVE_ARG(WaitHandler) handler)
  {
